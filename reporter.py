...
usability_tests = [
    ...
    {'test_name': 'Jitter above 25%',
     'test': lambda data: (float(data['mdev'].strip().strip('ms').strip()) / float(data['avg']) ) > 0.25,
     'problem_level': 1,
     'data_keys_to_report': ['mdev', 'avg']},
    {'test_name': 'Jitter above 50%',
     'test': lambda data: (float(data['mdev'].strip().strip('ms').strip()) / float(data['avg']) ) > 0.5,
     'problem_level': 2,
     'data_keys_to_report': ['mdev', 'avg']},
    {'test_name': 'Jitter above 75%',
     'test': lambda data: (float(data['mdev'].strip().strip('ms').strip()) / float(data['avg']) ) > 0.75,
     'problem_level': 3,
     'data_keys_to_report': ['mdev', 'avg']},
